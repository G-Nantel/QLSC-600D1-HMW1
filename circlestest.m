...
try
    ...
    whichScreen = 0;
     % Grey background, 800x800 window
    ...
    % Fixation cross parameters pixels
    fixCrossDimPix = 20;
    ...
    isiDuration = 0.5; % Inter-stimulus interval in seconds

    % Reference speed (radians per second)
    refSpeed = 2 * pi; % One full rotation per second

    % --- Condition set: angular speed offsets (rad/s) relative to a reference speed ---
    speedDiffs = [-pi, -pi/2, -pi/4, 0, pi/4, pi/2, pi];  % slower .. equal .. faster
    nLevels    = numel(speedDiffs);

    % --- Trial allocation ---
    trialsPerLevel = 2;% 2 trials per condition
    nTrials = nLevels * trialsPerLevel;   % total = 7 * 2 = 14,(2 per level)

    % Distribute 14 trials across levels
    % --- Build a balanced condition index vector then shuffle it ---
    % trialOrder holds indices 1..nLevels; use them to index into speedDiffs
