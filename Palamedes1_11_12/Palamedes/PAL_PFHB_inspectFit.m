...
for loop = 1:number
    ...
    if hiDensityRegion
        ...
        for paramI = 4:-1:1
            ...
            if isfield(pfhb.samples,strcat(param,'_actual'))
                paramsSamples.(paramNamesFull{paramI}) = reshape(pfhb.samples.(strcat(param,'_actual'))(:,:,c,s),[pfhb.engine.nsamples.*pfhb.engine.nchains,1]);
            else
                if isfield(pfhb.samples,param)
                    paramsSamples.(paramNamesFull{paramI}) = reshape(pfhb.samples.(param)(:,:,c,s),[pfhb.engine.nsamples.*pfhb.engine.nchains,1]);
                else
                    paramsSamples.(paramNamesFull{paramI}) = pfhb.model.(param).val.*ones(pfhb.engine.nsamples.*pfhb.engine.nchains,1);
                end
            end
            if paramI == 2
                paramsSamples.beta = 10.^paramsSamples.beta;
            end
            if paramI == 3 && pfhb.model.gammaEQlambda
                paramsSamples.gamma = paramsSamples.lambda;
            else
                paramsSamples.(paramNamesFull{paramI}) = repmat(paramsSamples.(paramNamesFull{paramI}),[1,length(xloc)]);
            end
        end

        y_s = PF(paramsSamples,repmat(xloc,[pfhb.engine.nsamples.*pfhb.engine.nchains,1]));
        bound = [0 1];
        if pfhb.model.g.Nc == 0 && pfhb.model.gammaEQlambda == false
            bound(1) = pfhb.model.g.val;
        end
        if pfhb.model.l.Nc == 0
            bound(2) = 1 - pfhb.model.l.val;
        end
        for index = 1:length(xloc)
            [grid, pdf] = PAL_kde(y_s(:,index),bound,'q');
            temp = PAL_hdi(grid,pdf,.68);
            hdi(index,:) = [temp(1,1) temp(end,2)];
        end
        patch([xloc fliplr(xloc)], [hdi(:,1)' fliplr(hdi(:,2)')], [.7 .7 .7],'edgecolor',[.7 .7 .7])
    end
    ...
    for ix = 1:length(x)
        if ~isinf(x(ix))
            plot(x(ix),y(ix)./n(ix),'o','markersize',30*sqrt(n(ix)./max(n)),'color',[0.8 0 0.2],'linewidth',2,'markerfacecolor',[0.8 0 0.2]);
        end
    end
    ...
    if any(infiniteLim)
        ...
        if infiniteLim(1)
            ...
            plot(x(1),y(1)./n(1),'o','markersize',30*sqrt(n(1)./max(n)),'color',[0.8 0 0.2],'linewidth',2,'markerfacecolor',[0.8 0 0.2])
        end
        if infiniteLim(2)
            ...
            plot(x(end),y(end)./n(end),'o','markersize',30*sqrt(n(end)./max(n)),'color',[0.8 0 0.2],'linewidth',2,'markerfacecolor',[0.8 0 0.2])
        end
