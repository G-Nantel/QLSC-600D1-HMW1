...
%
% Purpose: introduce two probability density functions. Introduction of
%  PAL_pdfBeta allows us to put a beta prior on parameters in demos and
%  such (Matlab's 'betapdf' function is in the 'statistics and machine 
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Version 1.11.12 Release June 13, 2025
%
% Purpose: Introduce 'guided priors' into hierarchical Bayesian (PAL_PFHB) 
%  routines. Palamedes uses stimulus placement to generate not particularly
%  informative priors for the location ('threshold') and slope parameters
%  of PF. PAL_PFHB_drawViolins recoded from scratch (to prote ct the sanity
%  of NP). Warning is issued when user selects weibull or quick as form of 
%  the psychomteric function and advised to use their log versions (Gumbel, 
%  loqQucik) instead. Bug fixes.
%
%PAL_kde:
% Modified: Palamedes version 1.11.12 (NP)
%  Disregard infinite sample values (which apparently can occur) when
%  calculating kernel bandwidth
%
%PAL_PFHB_drawViolins:
% Modified: Palamedes version 1.11.12 (NP)
%  Recoded from scratch to save sanity of NP (too many conditionals in 
%  previous version). This fixed minor bug: previous version mislabeled 
%  axes under some conditions.
%
%PAL_PFHB_drawViolinsCondxParam:
% Introduced: Palamedes version 1.11.12 (NP)
%  Plots 'condition by parameter' violin plots. This functionality was
%  included in PAL_PFHB_drawViolins in previous versions.
%
%PAL_PFHB_drawViolinsCondxSubj:
% Introduced: Palamedes version 1.11.12 (NP)
%  Plots 'condition by subject' violin plots. This functionality was
%  included in PAL_PFHB_drawViolins in previous versions.
%
%PAL_PFHB_drawViolinsSubjxParam:
% Introduced: Palamedes version 1.11.12 (NP)
%  Plot 'subject by parameter' violin plots. This functionality was
%  included in PAL_PFHB_drawViolins in previous versions.
%
%PAL_PFHB_figureInits:
% Modified: Palamedes version 1.11.12 (NP)
...
%PAL_PFHB_fitModel:
% Modified: Palamedes version 1.11.12 (NP)
%  Changes only made in help text (explaining use of 'a_prior_width_factor'
%  and 'b_prior_width_factor' options). Actual functionality for this is
%  implemented in PAL_PFHB_setUpModel
%
%PAL_PFHB_getSummaryStats:
% Modified: Palamedes version 1.11.12 (NP)
%  Minor bug fix regarding indeces in linList.
%
%PAL_PFHB_guidedPriors:
% Introduced: Palamedes version 1.11.12 (NP)
%  Find reasonable, not-very-informative prios for location and 
%  log10(slope) parameters based on stimulus placement. See
%  www.palamedestoolbox.org/guidedpriors.html for more information.
%
%PAL_PFHB_inspectFit:
% Modified: Palamedes version 1.11.12 (NP)
%  Constrain high-density region to be greater than or equal to guess rate 
%  if guess rate is fixed. Constrain high-density region to be less than or 
%  equal to 1 - lapse rate if lapse rate is fixed. Minor bug fix.
%
%PAL_PFHB_inspectParam:
% Modified: Palamedes version 1.11.12 (NP)
%  When option 'correl' is selected, stop after all meaningful parameter
%  pairings are shown once.
%
%PAL_PFHB_organizeSamples:
% Modified: Palamedes version 1.11.12 (NP)
%  Include descriptive 'hyperparameter' in description for hyperparameters 
%  in 'paramList'
%
%PAL_PFHB_setupModel:
% Modified: Palamedes version 1.11.12 (NP)
%  Introduce 'guided prior' functionality (see
%  www.palamedestoolbox.org/guidedpriors.html)
%  Issue warning if user selects weibull or quick, suggest using gumbel or
%  logQuick instead.
%  Minor bug fixes: 
%   Only include first four characters in model.(param).priorFun
%   Do not create field 'model.(param).Nc' for hyperparameters
%   Other cosmetic bug fixes
%
%PAL_PFHB_writeScript:
% Modified: Palamedes version 1.11.12 (NP)
%  Use 'filesep' consistently throughout.
%
