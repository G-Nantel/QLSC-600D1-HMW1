...
if exist('betapdf.m','file')        %betapdf requires Matlab's statistics toolbox
    prior = PAL_pdfNormal(a,0,1).*PAL_pdfNormal(b,0,1).*betapdf(l,1,10);    %prior for psi-marginal method
else
    prior = PAL_pdfNormal(a,0,1).*PAL_pdfNormal(b,0,1);    %prior for psi-marginal method
end
...
for cond = 1:Ncond
    ...
    b_gen(cond,1:Nsubj) = .5+.25*randn(1)+randn(1,Nsubj)*.25;
end
...
[pfhb] = PAL_PFHB_fitModel(data,'engine',engine,'nsamples',15000,'gammaeqlambda',true);

PAL_PFHB_inspectParam(pfhb,'amu','condition',2);
PAL_PFHB_inspectParam(pfhb,'a','subject',1,'condition',1,'l','subject',1,'effect',1);
PAL_PFHB_inspectFit(pfhb,'subject',1,'condition',1,'centraltendency','mean');
